...
**Note**: This is not currently implemented, but a current work-around for thread-specific data storage is to store your data in the first message of the thread, and add `hiddenFrom:["ai", "user"]` to the message. Then just access your data with e.g. `let threadData = JSON.parse(oc.thread.messages[0].content);`.
