...
```bash
docker run -p 80:80 -e MODEL=bigscience/bloom-560m hyperonym/basaran:0.15.3
```
Change `bigscience/bloom-560m` to a model name from [this list](https://huggingface.co/models?pipeline_tag=text-generation), and you can change `0.15.3` to the latest version from [here](https://hub.docker.com/r/hyperonym/basaran/tags).

### Step 3
Add this line to that text box in your user settings:
```json5
{name:"bigscience/bloom-560m", endpointUrl:"http://127.0.0.1/v1/completions"}
```
